...
虎码输入法字根练习工具：纯静态 Web 应用（无框架、无构建步骤），含字根练习页和字根表查询页、拆分查询页，其中拆分查询还支持五笔的。
...
- 本地运行：`npx nodemon server.js` → 自动打开 `http://localhost:8081`（页面用相对路径 fetch 数据，直接开 file:// 不可用）
...
- `index.html`：练习页，支持**字根练习 / 单字练习**等多种模式（`DATA_FILES` 映射数据源，`switchMode()` 切换并清旧进度）。核心流程 `initApp()` → `loadData()` → `parseData()` → `nextRoot()`/`checkInput()`；答错进 `wrongRoots`，一轮结束自动复习错题；空格键一次性提示。单字编码（`data/单字编码.txt`）形如 `的\tu_`，`checkInput` 中结尾 `_`（为简码区分符）可省略输入。
...
- 单字编码格式：`文字\t编码`，其中编码最长为4码（例 `tjkf`），简码不足4码的后面加 `_`，实际输入时可忽略，编码末尾的数字可忽略。
